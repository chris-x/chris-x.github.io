...
Rcpp::sourceCpp("cpp/SIR/sir_c.cpp")
...
```{r, echo = FALSE}
 ...
 output$R0<-renderUI({
         paste("Reproduction number R0 = ", round(input$D_rate*input$C_rate*input$P_rate/100,3))
   })

 output$distPlot <-renderPlotly({
         # generate bins based on input$bins from ui.R

         df_1<-SIR_CF(input$C_rate*input$P_rate/100,1/input$D_rate,0.0025,0.1)

         p<-ggplot(data=df_1)+
             geom_line(size=1.2,aes(x = Time, y = Vals, color = Group))+
             scale_colour_manual(values=c(I="#ef3d59",S="#1a75ff",R="#4ab19d"))+
             ylab("% population")+
             xlab("Time (days)")+
             labs(color='')+
             theme_bw()
...
ui<-fluidPage(
 sidebarLayout(
         sidebarPanel(width = 3,chooseSliderSkin("Nice"),
                      sliderInput("C_rate",
                                  "Contacts per day:",
                                  min = 1,
                                  max = 20,
...
Alternative modelling approaches, such as agent-based dynamics taking place on networks, do not impose such unrealistic assumptions. These tools allow the careful modelling of the role infrastructure networks (train, bus, air), and places where people may *mix*, in conjunction with more realistic localised movement and contact patterns.
