...
For specialised mathematical tasks I have used __Matlab__ and __Mathematica__ (and the intrguing [Wolfram language](http://www.wolfram.com/language/fast-introduction-for-programmers/en/)). I experimented with [Julia](http://julialang.org/), in its (very) early days. 
...
[Rcpp](http://dirk.eddelbuettel.com/code/rcpp.html) is a R package enabling the *easy* use of compiled code within R. While the R environment makes extensive use of compiled C and Fortran, inlcuding compiled code in packages and user-defined functions was, until now, *painful*. It is difficult to overstate how workflow in R is extended by Rcpp. Along with the optimised matrix operations of the [Armadillo C++ library](http://arma.sourceforge.net/), available to R through the [RcppArmadillo](http://dirk.eddelbuettel.com/code/rcpp.armadillo.html) package, and the recent [RcppParallel](https://rcppcore.github.io/RcppParallel/) package, leveraging CPU multicore concurrency direct from compiled C++, R is a serious environment for numerical programming. A good introduction to RcppParallel is [this talk](https://www.rstudio.com/resources/webinars/rcppparallel-a-toolkit-for-portable-high-performance-algorithms/).
...
[Tidyverse](https://www.tidyverse.org/) is the term given to the suite of R packages that have significantly enhanced (to put it mildly) how one approaches the task of data analysis, data wrangling and data visualisation in R -- compared to the *base R* workflow of `data.frame` and `plot`. While data frames are still a core feature of the language, and base R plot can plot almost anything (if you dedicate enough time to the task), the tidyverse philosophy significantly reduces the time (and effort) it takes to produce those *interesting* and *complex* analyses base R struggles with. Packages such as `dplyr` and `purrr` significally enhance base R capabilities 
...
I first used [Python](https://www.python.org/) in 2011, and enjoy using it when the need arises. Python is a simple and readable language to work with, and is sufficiently distinct from R and C++ to make learning and developing with it interesting and useful (for me). Although not specifically designed for statistical data analysis, Python has excellent support for it via numerous modules, such as [pandas](http://pandas.pydata.org/).

For statistical / machine learning, the well-known [scikit-learn](http://scikit-learn.org/stable/) is excellent, and there is Python support for deep learning via [TensorFlow](https://www.tensorflow.org/) and [Keras](https://keras.io/) (now subsumed within [TensorFlow 2](https://www.tensorflow.org/api/stable)).

<br>

# R v Python?

See [this excellent video](https://blog.dominodatalab.com/video-huge-debate-r-vs-python-data-science/) for a comparison of Python and R in terms of data science. 

The answer is, of course, both are excellent and neither one is perfect. Developers, like anyone else, tend to suffer from a cognitive bias in the form of an over-reliance on familiar tools. This situation is nicely summed up by a quote often attributed to Abraham Maslow: _'...it is tempting, if the only tool you have is a hammer, to treat everything as if it were a nail'_.
