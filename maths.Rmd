...
GEdgedf<-data.frame(from=GEdge[,1],to=GEdge[,2],weight=rep(1,nrow(GEdge)))
GEdgedf$color<-rep("purple",nrow(GEdge))

Gnodes <- data.frame(id= as.numeric(V(igg)$name))
Gnodes$value<-degree(igg)
grps <- cut(Gnodes$value,breaks=10,labels=FALSE)
Gnodes$color<-colfunc(10)[grps]

#Gnodes$group <- cut(Gnodes$value,breaks=10,labels=FALSE)
visNetwork(Gnodes,GEdgedf)
```
